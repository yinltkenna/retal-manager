...
using IdentityService.Infrastructure.Data.Seed;
...
// Add exception handling middleware
    ...
    if (app.Environment.IsDevelopment())
    {
        app.UseSwagger(c =>
        {
            ...
            {
                swagger.Servers = [
                    new() { Url = "http://localhost:5001", Description = "Chạy trực tiếp (No Gateway)" },
                    new() { Url = "http://localhost:5000/identity", Description = "Chạy qua Gateway" }
                ];
            });
        });
        app.UseSwaggerUI(c =>
